...
const pages = [
  ...
  { name: "Edit User Information", href: "#", current: true },
];
...
export default {
    ...
    methods: {
        ...
        async update(){
            ...
            .then((response) => {
                ...
                if (this.selectedRole){
                    this.changeRole(this.selectedRole.id);
                }else{
                    Swal.fire({
        ...
        changeRole(role_id){
            ...
            axios.put(process.env.VUE_APP_API_URL+`/user-management/update-user/`+ this.current_user.id +`/role/`+ role_id,{},
                { headers: authHeader() })
            .then((response) => {
